...
class AsyncWaremaHub extends BaseHub {
	...
	async getHubInfo() {
		this.status = await this.request('info');
		return this.status;
	}
	...
	async getDevices() {
		...
		LOGGER.debug('serial number of hub controller: ' + this.status.serialNumber);
		const response = await this.mb8Read(42, 0, parseInt(this.status.serialNumber), 62 * 20);
	...
	async request(path = 'info') {
		...
		try {
			const response = await this.session.get(`/${path}`, { timeout: 10000, responseType: 'arraybuffer' }); // Assuming 10 seconds timeout
	...
	async post(message = '', path = 'postMessage', base64Data = false) {
		LOGGER.debug(`Post message: ${message}`);
		const data = encode(message);

		try {
			const response = await this.session.post(`/${path}`, data, { timeout: 10000, responseType: 'arraybuffer' }); // Assuming 10 seconds timeout
			return super._processResponse(response.data);
		} catch (error) {
			// Handle errors appropriately, e.g., logging, retries
