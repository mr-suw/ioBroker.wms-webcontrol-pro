...
class AsyncWaremaDevice extends BaseWaremaDevice {
	constructor(hub, serialNumber, name, channel) {
		...
		LOGGER.debug(
			'WmsApi: AsyncWaremaDevice constructed. Check properties hub.webAddress ' + super.getHub().webAddress,
		);
	}
	...
	_setStatus(command = 3, setting0 = 255, setting1 = 255, setting2 = 255, setting3 = 255) {
		return super
			...
			.catch((error) => {
				console.error('WmsApi: Error setting device status:', error);
			});
	}

	_getStatus() {
		LOGGER.debug('WmsApi: Check super access to property serial number: ' + super.getSerialNumber());
		...
		return super
			...
			.then((response) => {
				...
				LOGGER.debug('WmsApi: raw response: ' + fields + ', length:' + fields.byteLength);
				...
				if (fields.byteLength >= 4) {
					...
					LOGGER.debug('WmsApi: integer response: ' + f0 + ', ' + f1 + ', ' + f2 + ', ' + f3);

					return [f0, f1, f2, f3];
				} else {
					throw 'WmsApi: Device response field values not available';
				}
			})
			.catch((error) => {
...
class AsyncGeneralBlind extends AsyncWaremaDevice {
	...
	async getPosition() {
		LOGGER.debug('WmsApi: AsyncGeneralBlind get position processing started.');
		try {
			const data = await super._getStatus();
			this.generalBlindStatus = new GeneralBlindStatus(...data);
		} catch (error) {
			LOGGER.debug('WmsApi: Error getting blind position ' + error.message);
...
class AsyncVenetianBlind extends AsyncWaremaDevice {
	...
	setTilt(tilt) {
		if (!this.venetianBlind._validTilt(tilt)) {
			throw new Error(
				`WmsApi: Tilt must be between ${VenetianBlind._minimum_tilt} and ${VenetianBlind._maximum_tilt}`,
			);
		}
	...
	setPosition(position, tilt) {
		if (!this.venetianBlind._validPosition(position)) {
			throw new Error(
				`WmsApi: Position must be between ${VenetianBlind._minimum_position} and ${VenetianBlind._maximum_position}`,
			);
		}
	...
	async getPosition() {
		LOGGER.debug('WmsApi: AsyncVenetianBlind get position processing started.');
		try {
			const data = await super._getStatus();
